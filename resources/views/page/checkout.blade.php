...
<div class="content">
    ...
    <form action="{{ route('order.confirm') }}" method="POST" id="checkoutForm">
        ...
        <div class="section-pink">
            <div class="produk-item">
                ...
                <div class="produk-detail">
                    <h4>Produk Undangan</h4>
                    {{-- Asumsi harga satuan 10.000, sesuaikan jika ada data harga dari database/session --}}
                    @php $hargaSatuan = 10000; @endphp
        ...
        <div class="informasi">
            ...
            <div class="info-item"><span>Nama Wanita</span><span>{{ $invitation['bride_name'] ?? '-' }}</span></div>
            <div class="info-item"><span>Tanggal Akad</span><span>{{ $invitation['akad_date'] ?? '-' }}</span></div>
            <div class="info-item"><span>Waktu Akad</span><span>{{ $invitation['akad_time'] ?? '-' }}</span></div>
            <div class="info-item"><span>Lokasi Akad</span><span>{{ $invitation['akad_location'] ?? '-' }}</span></div>
            <div class="info-item"><span>Tanggal Resepsi</span><span>{{ $invitation['event_date'] ?? '-' }}</span></div>
            <div class="info-item"><span>Waktu Resepsi</span><span>{{ $invitation['event_time'] ?? '-' }}</span></div>
            <div class="info-item"><span>Lokasi Resepsi</span><span>{{ $invitation['location_maps'] ?? '-' }}</span></div>
        </div>
        ...
        <div class="pengiriman">
            ...
            <select name="shipping_cost" id="shipping_cost" required style="width: 100%; padding: 10px; margin-top: 10px; border-radius: 5px; border: 1px solid #ccc; font-family: 'Poppins', sans-serif;">
                ...
                @else
                    @foreach($costs as $cost)
                        ...
                        <option value="{{ $cost['cost'] }}" data-harga="{{ $cost['cost'] }}">
                            {{ $cost['name'] }} ({{ $cost['service'] }}) - Rp {{ number_format($cost['cost'], 0, ',', '.') }}
                            (Estimasi: {{ $cost['etd'] }})
                        </option>
                    @endforeach
                @endif
        ...
        <div class="catatan" style="background: #fff; padding: 15px; border-radius: 8px; margin-bottom: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            ...
            <div class="catatan-item" style="display: flex; justify-content: space-between; font-weight: bold; font-size: 1.1em; color: #b0435e;">
        ...
        <div class="payment">
            ...
            <select name="payment_method" required style="width: 100%; padding: 10px; margin-top: 10px; border-radius: 5px; border: 1px solid #ccc; font-family: 'Poppins', sans-serif;">
        ...
        <div class="checkout-bar">
            ...
            <button type="submit" style="background-color: #b0435e; color: white; border: none; padding: 10px 20px; border-radius: 5px; font-weight: bold; cursor: pointer;">
                Checkout
            </button>
        </div>
